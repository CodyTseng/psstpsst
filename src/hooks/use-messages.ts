...
class ReplyTargetPageCache {
  readonly #messages: Record<string, MessageRow> = {};
  readonly #requestedIds = new Set<string>();

  constructor(readonly sessionKey: string) {}

  get(messageId: string): MessageRow | undefined {
    return this.#messages[messageId];
  }

  request(messageId: string): boolean {
    if (this.#requestedIds.has(messageId)) return false;
    this.#requestedIds.add(messageId);
    return true;
  }

  allowRetry(messageIds: readonly string[]): void {
    for (const messageId of messageIds) this.#requestedIds.delete(messageId);
  }

  add(rows: readonly MessageRow[]): boolean {
    let changed = false;
    for (const row of rows) {
      if (this.#messages[row.id] === row) continue;
  ...
  values(): Record<string, MessageRow> {
...
export type PaginatedMessages = {
  ...
  /** Reply targets outside the retained window, fetched once with their source
   * page rather than reactively from FlatList viewability changes. */
  replyTargetsById: Record<string, MessageRow>;
...
export function useMessages(
  ...
  const replyTargetCache = useMemo(
    () => new ReplyTargetPageCache(sessionKey),
    [sessionKey],
  );
  const [, bumpReplyTargetVersion] = useState(0);
  ...
  const hydrateReplyTargets = useCallback(async (pageRows: readonly MessageRow[]) => {
    if (!accountPubkey || pageRows.length === 0) return;
    const pageById = new Map(pageRows.map((row) => [row.id, row]));
    const resolvedTargets: MessageRow[] = [];
    const targetIds: string[] = [];
    for (const row of pageRows) {
      const targetId = row.replyToId;
      if (row.kind === 7 || !targetId || replyTargetCache.get(targetId)) continue;
      const pageTarget = pageById.get(targetId);
      if (pageTarget) {
        resolvedTargets.push(pageTarget);
        continue;
      }
      if (!replyTargetCache.request(targetId)) continue;
      targetIds.push(targetId);
    }
    if (targetIds.length === 0 && resolvedTargets.length === 0) return;
    ...
    if (targetIds.length > 0) {
      try {
        ...
        replyTargetCache.allowRetry(targetIds);
      }
    }
    if (
      !mountedRef.current ||
      activeSessionRef.current !== replyTargetCache.sessionKey ||
      resolvedTargets.length === 0
    ) {
      return;
    }
    if (!replyTargetCache.add(resolvedTargets)) return;
    startTransition(() => {
      bumpReplyTargetVersion((version) => version + 1);
    });
  }, [accountPubkey, replyTargetCache]);

  // A warmed tail can paint before the live query resolves. Hydrate its bounded
  // page independently; the requested-ID set deduplicates the later live page.
  useEffect(() => {
    if (!liveDataEnabled) return;
    void hydrateReplyTargets(
      tailReady
        ? tail.data.slice(0, MESSAGE_HISTORY_FETCH_SIZE)
        : tailRows,
    );
  }, [hydrateReplyTargets, liveDataEnabled, tail.data, tailReady, tailRows]);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      db
        ...
        .limit(MESSAGE_HISTORY_FETCH_SIZE + 1),
    ]).then(async ([olderResult, newerResult]) => {
      if (!active) return;
      await hydrateReplyTargets([
        ...olderResult.slice(0, MESSAGE_HISTORY_FETCH_SIZE),
        ...newerResult.slice(0, MESSAGE_HISTORY_FETCH_SIZE),
      ]);
      if (!active) return;
      updateAnchorWindow(anchorKey, () => ({
  ...
  }, [anchor, base, hydrateReplyTargets, liveDataEnabled, tailHasMore]);
  ...
  return {
    ...
    replyTargetsById: replyTargetCache.values(),
    loadOlder,
    loadNewer,
    hasMore,
    loadingOlder: anchor
