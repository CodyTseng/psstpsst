...
import { markChatClosePhase } from '@/lib/perf/chat-close';
...
export default function ChatPageRuntime() {
  ...
  useLayoutEffect(() => () => markChatClosePhase('layoutCleanupMs'), []);
  useEffect(() => () => markChatClosePhase('passiveCleanupMs'), []);
  useEffect(() => {
    const removeBefore = navigation.addListener('beforeRemove', () => {
      markChatClosePhase('removalRequestedMs');
    });
    const removeStart = navigation.addListener('transitionStart' as never,
      ((event: { data?: { closing?: boolean } }) => {
        if (event.data?.closing) markChatClosePhase('transitionStartedMs');
      }) as never);
    const removeEnd = navigation.addListener('transitionEnd' as never,
      ((event: { data?: { closing?: boolean } }) => {
        if (event.data?.closing) markChatClosePhase('transitionEndedMs');
      }) as never);
    return () => {
      removeStart();
      removeEnd();
    ...
    <AppScreen edges={[]}>
      ...
      >
        <ChatComposerPanelProvider>
          ...
          {liveDataReady && focused && isRelationshipEligible ? (
          ...
          <View style={{ flex: 1 }}>
            <ChatCleanupProbe scope="content">
            {messageRuntimeReady ? (
              <Reanimated.View
          ...
          <ChatCleanupProbe scope="composer">
          {!selectionMode ? (
            composerModel.mode === 'input' ? (
              <ChatInput
                draftKey={conversationKey}
                liveDataEnabled={secondaryDataReady && focused}
...
function ChatPageContent({
  ...
  const renderStartedAt = chatPerformanceNow();
  const insets = useSafeAreaInsets();
  ...
  useLayoutEffect(() => {
    const elapsedMs = Math.round(chatPerformanceNow() - renderStartedAt);
    if (elapsedMs >= 16 || !routeActive) {
      logChatPerformance('content.layoutCommit', {
        elapsedMs,
  ...
  const handleViewableMessageIdsChange = useCallback((ids: string[]) => {
  ...
  const secondaryQueryMessageIds = useMemo(() => {
    const retained = viewableMessageIds.filter((id) => loadedMessageIds.has(id));
    return retained.length > 0
      ? retained
      : messages.slice(-MESSAGES_PAGE_SIZE * 3).map((message) => message.id);
  }, [loadedMessageIds, messages, viewableMessageIds]);
  const deliveriesByMessageId = useMessageDeliveries(
    secondaryQueryMessageIds,
    secondaryDataReady && routeActive,
    `${accountPubkey ?? ''}\u0000${conversationKey}`,
  );
  ...
  const replyTargetIds = useMemo(() => {
    ...
    const scopedIds = new Set(secondaryQueryMessageIds);
    for (const message of messages) {
      if (
        scopedIds.has(message.id) &&
        message.replyToId &&
        !loadedMessageIds.has(message.replyToId)
      ) {
        out.add(message.replyToId);
      }
    }
    return Array.from(out);
  }, [loadedMessageIds, messages, secondaryQueryMessageIds]);
  const referencedById = useMessagesByIds(
    accountPubkey ?? '',
    replyTargetIds,
    secondaryDataReady && routeActive,
    `${accountPubkey ?? ''}\u0000${conversationKey}`,
  ...
  return (
    <>
      ...
      <View ref={contentRef} collapsable={false} style={{ flex: 1 }}>
        <ChatCleanupProbe scope="messageList">
        <MessageList
          ...
          onViewableMessageIdsChange={handleViewableMessageIdsChange}
        />
        </ChatCleanupProbe>
